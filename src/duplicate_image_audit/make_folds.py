...
for fold_idx, (train_idx, test_idx) in enumerate(sgkf.split(df, df["label"], df["group_id"])):
    ...
    train_df = outer_train.iloc[tr_i].copy()
    val_df = outer_train.iloc[va_i].copy()
    ...
    for part, role in ((train_df, "train"), (val_df, "val"), (test_df, "test")):
        part["fold"] = fold_idx
        part["role"] = role

    all_folds.append(pd.concat([train_df, val_df, test_df]))
